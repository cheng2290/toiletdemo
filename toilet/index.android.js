...
import ToiletPage from './common_views/toiletPage'
import ReadPage from './common_views/readPage'
import WeatherPage from './common_views/weatherPage'
import SettingPage from './common_views/settingPage'
...
export default class toilet extends Component {
    ...
    tabBarView() {
        return (
            <TabNavigator tabBarStyle={styles.tab}>
                {this.renderTabView('卫生间', 'toilet', '头条板块', true)}
                {this.renderTabView('阅读', 'read', '视频板块', false)}
                {this.renderTabView('天气', 'weather', '关注板块', false)}
                {this.renderTabView('设置', 'setting', '我的板块', false)}
            </TabNavigator>
        );
    }
    ...
    renderTabView(title, tabName, tabContent, isBadge) {
        ...
        //renderIcon={()=><Image style={styles.tabIcon} source={{uri: img_base64, scale: 3}}/>}
        {/* renderIcon={()=><Image style={styles.tabIcon} source={tabNomal}/>}
         renderSelectedIcon={()=><Image style={styles.tabIcon} source={tabPress}/>}*/}
        return (
            <TabNavigatorItem
                title={title}
                renderIcon={()=><Image style={styles.tabIcon} source={{uri: img_base64, scale: 3.5}}/>}
